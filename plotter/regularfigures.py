...
def NCDR_surface(figfullpath, lat, lon, u10 = None, v10 = None, mslp = None, cwv = None, thick_1000_500 = None, title = "", xlim = [108, 128], ylim = [15, 35], ticksitvl = [None, None]):
    MP = MapPlotter(figsize=figsize)
    PB2 = Paintbox_2D(field=dict(u10=u10,v10=v10,mslp=mslp,cwv=cwv,geoheight_thick=thick_1000_500), X=lon, Y=lat, fig=MP.fig, ax=MP.ax, ft=MP.fontsize)
    MP.coastlines()
    ...
    PB2.contour(varname="geoheight_thick", levels=np.arange(15)*6+960, linewidths=MP.linewidth, clabel=True)
    MP.grid()
    MP.savefig(figfullpath)
